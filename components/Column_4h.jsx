...
export default function Column4h({
  snapshot,
  probs,
  counts,
  rangePred,
  ...
  const getMinutesUntilNext4HSession = () => {
    if (!timestamp) return "—"

    try {
      const now = new Date(timestamp)
      const sessions = [2, 6, 10, 14, 18, 22] // 2am, 6am, 10am, 2pm, 6pm, 10pm
      const currentHour = now.getHours()

      // Find next session
      let nextSession = sessions.find((hour) => hour > currentHour)
      if (!nextSession) {
        nextSession = sessions[0] + 24 // Next day's first session
      }

      const nextSessionTime = new Date(now)
      if (nextSession >= 24) {
        nextSessionTime.setDate(nextSessionTime.getDate() + 1)
        nextSessionTime.setHours(nextSession - 24, 0, 0, 0)
      } else {
        nextSessionTime.setHours(nextSession, 0, 0, 0)
      }

      const diffMs = nextSessionTime.getTime() - now.getTime()
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h2 className="font-serif font-bold text-xl text-card-foreground">4H Analytics</h2>
          <span className="text-sm text-muted-foreground">| {getMinutesUntilNext4HSession()} till next session</span>
        </div>
      </div>
        ...
        <div className="grid gap-4">
          {Object.entries(filters).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between">
              <Label htmlFor={`4h-${key}`} className="text-sm font-medium text-card-foreground">
                {label[key] || key}
              </Label>
              <Switch
                id={`4h-${key}`}
